...
function Process() {
  return <section id="o-treningu" className="relative overflow-hidden bg-background section-space"><Network className="opacity-20" />
    <div className="section-shell relative"><div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:items-end">
      ...
      <div className="relative grid gap-0 border-l border-border md:grid-cols-3 md:border-l-0 md:border-t">
        {pageContent.process.steps.map((step, i) => <div key={step} className="reveal-up relative flex min-h-28 items-center gap-5 border-b border-border px-5 py-5 md:block md:border-b-0 md:border-r">
...
function Diagnostics() {
  ...
  return <section id="diagnostyka" className="bg-card section-space"><div className="section-shell">
    <div className="grid gap-12 lg:grid-cols-2 lg:items-center">
      <div className="image-reveal relative min-h-[34rem] overflow-hidden"><img src={media.diagnostics.src} alt={media.diagnostics.alt} loading="lazy" width={media.diagnostics.width} height={media.diagnostics.height} className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-[linear-gradient(0deg,var(--card),transparent_55%)]"/><div className="absolute bottom-7 left-7 rounded-full border border-primary/50 bg-background/75 p-5 backdrop-blur"><Activity className="size-8 text-primary" /></div></div>
      <div className="reveal-up"><SectionLabel>{pageContent.diagnostics.label}</SectionLabel><h2 className="display-title text-5xl md:text-7xl">{pageContent.diagnostics.title}<br/><span className="text-primary">{pageContent.diagnostics.titleAccent}</span><br/>{pageContent.diagnostics.titleLine}</h2><p className="mt-6 max-w-xl leading-7 text-muted-foreground">{pageContent.diagnostics.description}</p>
        <div className="mt-10 grid gap-px bg-border sm:grid-cols-2">{items.map((item,i) => { const I = iconMap[item.icon]; const { title, text } = item; return <div key={String(title)} className="bg-card p-6"><div className="flex items-center justify-between"><I className="size-6 text-primary"/><span className="font-display text-3xl text-foreground/10">0{i+1}</span></div><h3 className="mt-5 text-xl font-bold uppercase">{String(title)}</h3><p className="mt-2 text-sm text-muted-foreground">{String(text)}</p></div>})}</div>
      </div>
    </div>
  </div></section>;
}
...
function Control() {
  ...
  return <section className="relative overflow-hidden bg-ink section-space"><img src={media.control.src} alt={media.control.alt} loading="lazy" width={media.control.width} height={media.control.height} className="absolute inset-0 h-full w-full object-cover opacity-35"/><div className="absolute inset-0 bg-[linear-gradient(90deg,var(--ink)_25%,color-mix(in_oklab,var(--ink)_75%,transparent))]"/><Network />
    <div className="section-shell relative"><div className="max-w-3xl reveal-up"><SectionLabel>{pageContent.control.label}</SectionLabel><h2 className="display-title text-5xl md:text-8xl">{pageContent.control.title}</h2><p className="mt-6 max-w-2xl text-lg leading-8 text-foreground/75">{pageContent.control.description}</p></div>
      <div className="mt-10 grid md:mt-12 gap-px bg-foreground/15 md:grid-cols-3">{items.map((item,i)=>{const I=iconMap[item.icon]; const { title, text } = item;return <div key={String(title)} className="bg-ink/75 p-7 backdrop-blur-sm"><I className="size-7 text-gold"/><p className="mt-8 font-display text-sm text-primary">0{i+1}</p><h3 className="mt-2 text-3xl font-bold uppercase">{String(title)}</h3><p className="mt-2 text-sm text-muted-foreground">{String(text)}</p></div>})}</div>
    </div>
  </section>;
}
...
function Pillars() {
  ...
  return <section id="wiedza" className="section-space"><div className="section-shell"><div className="reveal-up text-center"><SectionLabel>{pageContent.pillars.label}</SectionLabel><h2 className="display-title mx-auto max-w-4xl text-5xl md:text-8xl">{pageContent.pillars.title}<br/>{pageContent.pillars.titleLine}</h2></div>
    <div className="mt-10 grid md:mt-12 gap-px bg-border md:grid-cols-4">{pillars.map((item,i)=>{const I=iconMap[item.icon]; const { title, color, text } = item;return <article key={String(title)} className="group relative min-h-72 overflow-hidden bg-card p-7"><span className="absolute right-4 top-1 font-display text-7xl font-black text-foreground/5">0{i+1}</span><I className={`size-9 ${String(color)}`}/><h3 className="mt-20 text-3xl font-bold uppercase">{String(title)}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{String(text)}</p><div className="absolute inset-x-0 bottom-0 h-1 origin-left scale-x-0 bg-primary transition-transform duration-500 group-hover:scale-x-100"/></article>})}</div>
...
function Programs() {
  return <section id="programy" className="bg-card section-space"><div className="section-shell"><div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-end"><div><SectionLabel>{pageContent.programs.label}</SectionLabel><h2 className="display-title text-6xl md:text-8xl">{pageContent.programs.title}<br/><span className="text-primary">{pageContent.programs.titleAccent}</span></h2></div><p className="max-w-sm text-sm leading-7 text-muted-foreground">{pageContent.programs.description}</p></div>
    <div className="mt-10 grid md:mt-12 gap-px bg-border md:grid-cols-2 lg:grid-cols-3">{pageContent.programs.items.map(([num,title,text],i)=><article key={title} className={`group min-h-60 bg-card p-7 transition-colors hover:bg-muted ${i===5?"lg:bg-burgundy/25":""}`}><div className="flex items-center justify-between"><span className="font-display text-4xl font-bold text-primary">{num}</span><ArrowRight className="size-5 text-muted-foreground transition-transform group-hover:translate-x-1 group-hover:text-gold"/></div><h3 className="mt-14 text-2xl font-bold uppercase">{title}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground">{text}</p></article>)}</div>
  </div></section>;
}

function Performance() {
  return <section className="relative flex min-h-[88svh] items-center justify-center overflow-hidden"><img src={media.performance.src} alt={media.performance.alt} loading="lazy" width={media.performance.width} height={media.performance.height} className="absolute inset-0 h-full w-full object-cover object-center"/><div className="absolute inset-0 bg-background/45"/><div className="absolute inset-0 bg-[linear-gradient(90deg,color-mix(in_oklab,var(--cyan)_20%,transparent),transparent,color-mix(in_oklab,var(--burgundy)_28%,transparent))]"/><Network />
...
function Coach() {
  return <section id="trener" className="bg-card section-space"><div className="section-shell"><div className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
...
function Tailored() {
  ...
  return <section className="relative overflow-hidden section-space"><Network className="opacity-25"/><div className="section-shell relative"><SectionLabel>{pageContent.tailored.label}</SectionLabel><div className="grid gap-12 lg:grid-cols-[1.2fr_.8fr]"><h2 className="display-title text-6xl md:text-9xl">{pageContent.tailored.title}<br/><span className="text-primary">{pageContent.tailored.titleAccent}</span></h2><div><p className="font-display text-4xl font-bold uppercase leading-none text-gold md:text-5xl">{pageContent.tailored.statement}<br/><span className="text-foreground">{pageContent.tailored.statementAccent}</span></p><div className="mt-9">{differences.map((x,i)=><div key={x} className="flex items-center gap-4 border-b border-border py-4"><CircleDot className="size-4 text-primary"/><span className="font-display text-xl font-semibold uppercase">{x}</span><span className="ml-auto text-xs text-muted-foreground">0{i+1}</span></div>)}</div></div></div></div></section>;
}

function SocialProof() {
  const stats = pageContent.socialProof.stats;
  return <><section className="border-y border-border bg-ink py-12 md:py-14"><div className="section-shell grid grid-cols-2 gap-px bg-border md:grid-cols-4">{stats.map(([value,label])=><div key={label} className="bg-ink p-5 text-center md:p-8"><p className="font-display text-5xl font-black text-primary md:text-7xl">{value}</p><p className="mt-2 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground">{label}</p></div>)}</div><p className="section-shell mt-4 text-center text-[10px] uppercase tracking-[0.12em] text-muted-foreground">{pageContent.socialProof.statsNote}</p></section>
  <section className="bg-card section-space"><div className="section-shell"><SectionLabel>{pageContent.socialProof.label}</SectionLabel><h2 className="display-title max-w-4xl text-5xl md:text-8xl">{pageContent.socialProof.title}<br/>{pageContent.socialProof.titleLine}</h2><div className="mt-10 grid gap-5 md:mt-12 md:grid-cols-2">{pageContent.socialProof.testimonials.map((item, index) => <blockquote key={item.author} className={`border-l-2 bg-background p-8 ${index === 0 ? "border-primary" : "border-burgundy"}`}><p className="text-lg leading-8 text-foreground/70">{item.quote}</p><footer className="mt-7 text-xs font-bold uppercase tracking-[0.15em] text-gold">{item.author}</footer></blockquote>)}</div></div></section></>;
}
